...
function SWEP:Think()
	if CurTime() < self.Weapon.HurtTime then -- this way there is no way we miss the entity even though we are in-range but have missed the moment when it was supposed to hit the entity
		...
		if SERVER then
			if self.Owner.Class == "recon" and self.Owner.CloakOn then
...
function SWEP:PrimaryAttack()
	...
	timer.Simple(self.TimerDelay, function()
		...
		self.Owner:ViewPunch(Angle(math.random(self.Weapon.DamageToPlayers1, self.Weapon.DamageToPlayers2) * 0.3, math.random(-2, 2), math.random(-2, 2)))
