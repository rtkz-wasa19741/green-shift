...
SWEP.DropGrenadeType = "ta_incendiary"
			...
			if CurTime() > self.Weapon.ThrowDelay then
				...
				timer.Simple(0.25, function()
					...
					if SERVER then
						...
						if self.Owner:Team() == TEAM_COMBINE then
							self.Owner:EmitSound("npc/metropolice/vo/grenade.wav", 85, 100)
						end
...
function SWEP:PrimaryAttack()
	if self.Owner:GetEyeTrace().HitPos:Distance(self.Owner:GetShootPos()) <= 30 or self.Owner:GetAmmoCount("RPG_Round") == 0 then
		return
	end
